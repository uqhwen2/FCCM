...
def train_test_splitting(combine_x, combined_y, tau, T, test_size, seed, device):
    ...
    tau_train, tau_test, \
    T_train_, T_test = train_test_split(combine_x_np, combined_y_np, tau_np, T_np, test_size=test_size, random_state=seed)
...
# for seed in range(num_trial):
    ...
    combine_x, combined_y, tau, T \
        = generation(mean_1=0, std_1=1, func_1=torch.sin, mean_0=3, std_0=1, func_0=torch.cos, device=device)

    combine_x_train, combine_x_test, combined_y_train, combine_x_valid, \
    combined_y_valid, tau_test, T_train, \
    T_valid, T_test = train_test_splitting(combine_x, combined_y, tau, T, test_size, seed, device=device)
