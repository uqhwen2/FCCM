...
#    G.add_edges_from(edges)
    radius = 0.11
    threshold_1, threshold_0, threshold_10, threshold_01 = radius, radius, radius, radius  # current result based on all 0.11
...
#    print('Free Up','\n')
    ...
    for query_step in range(al_step):
        ...
        class_1_scatter = plt.scatter(
            class_1_data[:, 0], class_1_data[:, 1],
            color='red', alpha=0.1, marker='o', s=3, label="$t=1$"
        )

        # Plot points for Class 0
        class_0_scatter = plt.scatter(
            class_0_data[:, 0], class_0_data[:, 1],
            color='blue', alpha=0.1, marker='x', s=3, label="$t=0$", linewidths=0.5
        )
        ...
        #outer_contour = mlines.Line2D([], [], color='green', linestyle='--', label="Outer Contour")
        inner_contour = mlines.Line2D([], [], color='purple', linestyle='--', label="Overlap Region")

        # Combine scatter and contour handles
        handles = [
            class_1_scatter,  # Class 1 scatter handle
            class_0_scatter,  # Class 0 scatter handle
            #outer_contour,  # Outer contour handle
            inner_contour  # Inner contour handle
        ]

        # Add combined legend
        plt.legend(handles=handles, loc="upper center", fontsize=4, ncol=3)
...
#        for b in range(num_of_samples):
        ...
        if len(acquired_treated) != 0:
            ...
            plt.scatter(
                ...
                color='red', label="Selected", alpha=1.0, s=50, marker='*'
            )
            ...
            for i, point in enumerate(combine_x_train[best_node_]):
                plt.text(
                    ...
                    fontsize=5, color='black', ha='center', va='center'  # Styling
                )
        ...
        plt.savefig('data_acquisition_plot/dataset_overlap/data_overlap.pdf'.format(query_step), bbox_inches='tight')
