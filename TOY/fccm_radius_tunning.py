...
#                print('Counterfactual covered')
        ...
        # Let's assume save_tensor_normalized_distances is a torch.Tensor already loaded in your workspace
        # Index masks for sub_training and treated points
        mask_sub_training_1 = torch.tensor([i in idx_sub_training_1.tolist() for i in range(save_tensor_normalized_distances.shape[0])])
        mask_treated_index = torch.tensor([i in treated_index for i in range(save_tensor_normalized_distances.shape[0])])

        # Extract the sub-matrix of distances from sub_training_1 to all treated indices
        distances_sub_training_to_treated = save_tensor_normalized_distances[mask_sub_training_1][:, mask_treated_index]
        ...
        print(
            f"Number of points in treated_index covered by idx_sub_training_1 within distance 0.5: {round(coverage_11, 2)}")

        # Index masks for sub_training and control points
        mask_control_index = torch.tensor([i in control_index for i in range(save_tensor_normalized_distances.shape[0])])

        # Extract the sub-matrix of distances from sub_training_1 to all control indices
        distances_sub_training_to_control = save_tensor_normalized_distances[mask_sub_training_1][:, mask_control_index]
        ...
        print(f"Number of points in control_index covered by idx_sub_training_1 within distance 0.5: {round(coverage_10, 2)}")
        ...
        # Index masks for sub_training and treated points
        mask_sub_training_0 = torch.tensor([i in idx_sub_training_0.tolist() for i in range(save_tensor_normalized_distances.shape[0])])
        mask_control_index = torch.tensor([i in control_index for i in range(save_tensor_normalized_distances.shape[0])])

        # Extract the sub-matrix of distances from sub_training_1 to all treated indices
        distances_sub_training_to_control = save_tensor_normalized_distances[mask_sub_training_0][:, mask_control_index]
        ...
        print(
            f"Number of points in control_index covered by idx_sub_training_0 within distance 0.5: {round(coverage_00, 2)}")

        # Index masks for sub_training and control points
        mask_treated_index = torch.tensor([i in treated_index for i in range(save_tensor_normalized_distances.shape[0])])

        # Extract the sub-matrix of distances from sub_training_1 to all control indices
        distances_sub_training_to_treated = save_tensor_normalized_distances[mask_sub_training_0][:, mask_treated_index]
        ...
        print(f"Number of points in treated_index covered by idx_sub_training_0 within distance 0.5: {round(coverage_01, 2)}")

        coverage_list_00.append(coverage_00)
        coverage_list_01.append(coverage_01)

# Specify the file pathstep_1
file_path = 'text_results/coverage_visuals_{}/fccm_coverage_{}_r{}.csv'.format(args.alpha, args.alpha, args.radius)
