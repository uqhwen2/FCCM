...
parser.add_argument('--radius', type=float, default=0.11)
parser.add_argument('--path', type=str, default='')
args = parser.parse_args()

import json
# Read the configuration from the file
with open('experiments/config_probcover.json', 'r') as file:
...
# for seed in range(num_trial):
    ...
    print('Remaining treated samples:', len(idx_remaining_1))
...
#    G.add_nodes_from(range(normalized_distances.size(0)))s
    # Add edges based on the indices
#    edges = list(zip(cols.cpu().numpy(), rows.cpu().numpy()))  # direction from cols to rows
#    G.add_edges_from(edges)

    threshold_1, threshold_0, threshold_10, threshold_01 = 0.11, 0.11, args.radius, args.radius
...
#    print('Free Up','\n')
    ...
    for query_step in range(al_step):
        ...
        plt.scatter(train_x_0[:, 0].cpu().numpy(), train_x_0[:, 1].cpu().numpy(), color='blue', label="Class 0 - Training", alpha=0.1, s=10)

        # Plot training data for Class 1
        plt.scatter(train_x_1[:, 0].cpu().numpy(), train_x_1[:, 1].cpu().numpy(), color='red', label="Class 1 - Training", alpha=0.5, s=10)

        # Plot remaining data for Class 0 (points not yet queried)
        plt.scatter(combine_x_train[idx_remaining_0][:, 0].cpu().numpy(), combine_x_train[idx_remaining_0][:, 1].cpu().numpy(),
                    color='blue', label="Class 0 - Remaining", alpha=0.1, s=10, marker='x')

        # Plot remaining data for Class 1 (points not yet queried)
        plt.scatter(combine_x_train[idx_remaining_1][:, 0].cpu().numpy(), combine_x_train[idx_remaining_1][:, 1].cpu().numpy(),
                    color='red', label="Class 1 - Remaining", alpha=0.1, s=10, marker='x')

        # Set plot title and labels
        plt.title("Train and Remaining Data Points by Class")
        ...
        job_dir_path = Path('saved_models/method_{}_alpha{}_r10_{}/seed_{}/step_{}'.format(
            config.get("acquisition_function"), args.alpha, args.radius, seed, 0)
        )
...
file_path = 'text_results/probcover/pehe_probcover_seed{}.csv'.format(args.seed)
